...
public class DefaultApiRequestManager extends AbstractControlComponent implements ApiRequestManager {
     ...
     * @throws ???
     */
    public String process(Message message) throws MiddlewareException {
        ...
        try {
            ...
            throw new MiddlewareException(String.format(
                    "Failed to process message %s of type %s from the client %s: %s",
                    message.getMetadata().getMessageID().get(), messageTypes, clientId, e.getMessage()));
        }
    }
    ...
    public synchronized void updateCallback(Client client) throws MiddlewareException {
        ...
        try {
            ...
            throw new MiddlewareException(e, "Failed to update callback for the client %s: %s",
        ...
        if (clients.isEmpty()) {
            ...
            for (Client client : clients) {
                ...
                try {
                    ...
                    throw new MiddlewareException(String.format("Failed to restore callback for client %s: %s",
    ...
    private void restoreSubscriptions(Subscription subscription) throws MiddlewareException {
        ...
        if (devices.isEmpty()) {
            throw new MiddlewareException("No devices found corresponding to subscription " + subscription.getConversationId());
        }
